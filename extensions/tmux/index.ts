...
class TmuxFocusModal implements Component {
  ...
  // No polling timer — output is driven exclusively by pipe-pane streaming.
  ...
  private async initialize(): Promise<void> {
    try {
      this.unsubscribeStream = await subscribePaneOutput(this.target, (chunk) =>
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.on("session_start", async (_event, ctx) => {
    storedCtx = ctx;
    // Derive session name eagerly so watchers can use it.
    sessionName = deriveSessionName(ctx.cwd ?? process.cwd());
    sessionReady = false; // Will be confirmed on first use.
  });
  ...
  pi.registerCommand("tmux:focus", {
    description: "Open the tmux focus modal for a window: /tmux:focus [window]",
  ...
  pi.registerCommand("tmux:run", {
    description: "Run a command in a named tmux window (window closes when done): /tmux:run <window> <command>",
    handler: async (args, ctx) => runInWindow(args, ctx, { keep: false }),
  });

  pi.registerCommand("tmux:run:keep", {
    description: "Run a command in a named tmux window (keeps shell open when done): /tmux:run:keep <window> <command>",
  ...
  async function runInWindow(args: string | undefined, ctx: any, opts: { keep: boolean }): Promise<void> {
      ...
      if (!input) {
        ctx.ui.notify(`Usage: /tmux:run${opts.keep ? ":keep" : ""} <window> <command>\nExample: /tmux:run server npm run dev`, "warning");
        return;
      }
      const cwd = ctx.cwd ?? process.cwd();
      let sess: string;
